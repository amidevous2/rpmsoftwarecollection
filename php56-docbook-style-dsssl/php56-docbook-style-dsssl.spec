...
make install BINDIR=$DESTDIR%{_bindir} DESTDIR=$DESTDIR%{_prefix}/share/sgml/docbook/dsssl-stylesheets-%{version} MANDIR=$DESTDIR%{_mandir}
cd ..
ln -s dsssl-stylesheets-%{version} $DESTDIR%{_prefix}/share/sgml/docbook/dsssl-stylesheets
%{?scl:EOF}
...
%{_prefix}/share/sgml/docbook/dsssl-stylesheets-%{version}
%{_prefix}/share/sgml/docbook/dsssl-stylesheets
...
do
  %{_bindir}/install-catalog --add $centralized \
    %{_prefix}/share/sgml/docbook/dsssl-stylesheets-%{version}/catalog \
    > /dev/null 2>/dev/null
done
...
if [ "$1" = "0" ]; then
  for centralized in /etc/sgml/*-docbook-*.cat
  do
    %{_bindir}/install-catalog --remove $centralized %{_prefix}/share/sgml/docbook/dsssl-stylesheets-%{version}/catalog > /dev/null 2>/dev/null
  done
fi
exit 0
%{?scl:EOF}
