%{?scl:%scl_package openjade}
%{!?scl:%global pkg_name openjade}
...
Provides: %{?scl_prefix}jade = %{version}-%{release}
...
%patch -P 0 -p1
%patch -P 1 -p1
%patch -P 2 -p1 -b .deplibs
%patch -P 3 -p1 -b .nola
%patch -P 4 -p1 -b .gcc46
%patch -P 5 -p1 -b .getopt
...
cp dsssl/catalog $RPM_BUILD_ROOT/%{_datadir}/sgml/%{pkg_name}-%{version}/
cp dsssl/{dsssl,style-sheet,fot}.dtd $RPM_BUILD_ROOT/%{_datadir}/sgml/%{pkg_name}-%{version}/

# add unversioned/versioned catalog and symlink
mkdir -p $RPM_BUILD_ROOT/etc/sgml
cd $RPM_BUILD_ROOT/etc/sgml
touch %{pkg_name}-%{version}-%{release}.soc
ln -s %{pkg_name}-%{version}-%{release}.soc %{pkg_name}.soc
cd -
...
set -ex
%{?ldconfig}
install-catalog --add /etc/sgml/%{pkg_name}-%{version}-%{release}.soc \
    %{_datadir}/sgml/%{pkg_name}-%{version}/catalog >/dev/null 2>/dev/null || :
%{?scl:EOF}
...
%{?scl:scl enable %{scl} - << \EOF}
set -ex
install-catalog --remove /etc/sgml/%{pkg_name}-%{version}-%{release}.soc \
    %{_datadir}/sgml/%{pkg_name}-%{version}/catalog >/dev/null 2>/dev/null || :
...
touch %{_sysconfdir}/sgml/%{pkg_name}-%{version}-%{release}.soc 
%{?scl:EOF}
...
%verify(not size filedigest mtime) %{_sysconfdir}/sgml/%{pkg_name}-%{version}-%{release}.soc
/etc/sgml/%{pkg_name}.soc
%{_bindir}/*
%{_libdir}/*.so.*
%{_mandir}/*/*
%{_datadir}/sgml/%{pkg_name}-%{version}
...
- /usr/lib/sgml -> /usr/share/sgml/%%{pkg_name}-%%{version}.
